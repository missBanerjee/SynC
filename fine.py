...
def compute_mean_var_test(model,dino_head, train_loader):
...
def average_covariance_score_for_text_neighbor(texts: np.ndarray, covs: np.ndarray):
    """
    For each class i, find its top-1 neighbor j in text-space and return the average
    covariance similarity score between i and its text-selected neighbor.
    
    Args:
        texts: np.ndarray of shape (n, d_text) - class name embeddings
        covs: np.ndarray of shape (n, D, D) - class covariance matrices
    
    Returns:
        avg_score: float, the average covariance similarity score
    """
    # Number of classes
    n = texts.shape[0]
    
    # 1) Compute text similarity matrix (cosine)
    T_norm = texts / np.linalg.norm(texts, axis=1, keepdims=True)
    S = T_norm @ T_norm.T
    np.fill_diagonal(S, -np.inf)  # exclude self
    
    # 2) Compute covariance-distance, then similarity
    flat_covs = np.stack([m.ravel() for m in covs], axis=0)  # shape (n, D*D)
    F_vals = pdist(flat_covs, metric='euclidean')            # pairwise Frobenius distances
    tau = np.median(F_vals)
    C_vals = np.exp(-F_vals / tau)                           # convert to similarity
    C_full = squareform(C_vals, checks=False)                # full covariance similarity matrix
    np.fill_diagonal(C_full, 0)  # set self-similarity to zero or exclude as needed
    
    # 3) Get top-1 text neighbor for each class
    nn_text = np.argmax(S, axis=1)  # shape (n,)
    
    # 4) Extract covariance similarity score for each (i, nn_text[i])
    cov_scores = np.array([C_full[i, nn_text[i]] for i in range(n)])
    
    # 5) Compute average score
    avg_score = np.mean(cov_scores)
    return avg_score


def validate_semantic_covariance_topk_alignment(texts: np.ndarray,
                                                 covs: np.ndarray,
                                                 k: int = 3):
    """
    texts: shape (n, d_text)
    covs:  shape (n, D, D)
    k:     number of top covariance neighbors to consider
    """
    n = texts.shape[0]

    # 1) Semantic‐similarity matrix (cosine)
    T_norm = texts / np.linalg.norm(texts, axis=1, keepdims=True)
    S = T_norm @ T_norm.T

    # 2) Covariance‐distance → similarity
    def frob_pairwise(mat_list):
        flat = np.stack([m.ravel() for m in mat_list], axis=0)
        return pdist(flat, metric='euclidean')

    F_vals = frob_pairwise(covs)
    tau = np.median(F_vals)
    C_vals = np.exp(-F_vals / tau)
    C_full = squareform(C_vals, checks=False)

    # 3) Identify top-1 text neighbor and top-k covariance neighbors
    np.fill_diagonal(S, -np.inf)
    np.fill_diagonal(C_full, -np.inf)

    nn_text = np.argmax(S, axis=1)
    nn_cov_topk = np.argsort(-C_full, axis=1)[:, :k]

    # 4) Top-k agreement: check if text neighbor is in covariance top-k
    agreement_topk = np.mean([nn_text[i] in nn_cov_topk[i] for i in range(n)]) * 100

    print(f"Top-1 text neighbor in Top-{k} covariance neighbors: {agreement_topk:.1f}%")
    return agreement_topk
...
def max_covariance_score_for_topk_text_neighbors(texts: np.ndarray, covs: np.ndarray, k: int = 3):
    """
    For each class i, find its top-k neighbors in text space and compute the maximum
    covariance similarity score between i and those neighbors.
    
    Args:
        texts: np.ndarray of shape (n, d_text) - class name embeddings
        covs: np.ndarray of shape (n, D, D) - class covariance matrices
        k:     int - number of top text neighbors to consider (default 3)
    
    Returns:
        max_scores_per_class: np.ndarray of shape (n,) with the maximum covariance similarity 
                              score for each class's top-k text neighbors
    """
    n = texts.shape[0]
    ...
    C_vals = np.exp(-F_vals / tau)                          
    C_full = squareform(C_vals, checks=False)               
    np.fill_diagonal(C_full, 0)  # optional: exclude self-similarity
    
    # 3) Get top-k text neighbors for each class
    nn_text_topk = np.argsort(-S, axis=1)[:, :k]  # shape (n, k)
    
    # 4) Compute per-class max covariance similarity
    max_scores_per_class = np.zeros(n)
    for i in range(n):
        neighbors = nn_text_topk[i]
        max_scores_per_class[i] = np.max([C_full[i, j] for j in neighbors])
    
    return max_scores_per_class



def average_covariance_score_for_topk_text_neighbors(texts: np.ndarray, covs: np.ndarray, k: int = 3):
    """
    For each class i, find its top-k neighbors in text space and compute the average
    covariance similarity score between i and its text-selected neighbors across all classes.
    ...
    # 4) Extract covariance similarity scores for each neighbor and average
    all_scores = []
    for i in range(n):
        for j in nn_text_topk[i]:
            all_scores.append(C_full[i, j])
    
    # 5) Compute overall average score
...
def validate_alignment_top3_with_labels(texts: np.ndarray,
                                        covs: np.ndarray,
                                        labels=None,
                                        k: int = 3):
    """
    For each class, find its top-1 text neighbor and top-k covariance neighbors,
    compute the agreement rate, and return neighbor labels.
    
    Args:
        texts: np.ndarray of shape (n, d_text)
        covs:  np.ndarray of shape (n, D, D)
        labels: list of length n of class labels (optional; defaults to indices)
        k: number of top covariance neighbors to consider (default 3)
    
    Returns:
        agreement_rate: float, percentage where text neighbor is in covariance top-k
        neighbors: list of dicts with keys:
            'class', 'text_neighbor', 'covariance_neighbors'
    """
    n = texts.shape[0]
    print("n",n)
    if labels is None:
    ...
    # 2) Covariance similarity
    flat_covs = np.stack([m.ravel() for m in covs], axis=0)
    F_vals = pdist(flat_covs, metric='euclidean')
    tau = np.median(F_vals)
    C_vals = np.exp(-F_vals / tau)
    C_full = squareform(C_vals, checks=False)
    np.fill_diagonal(C_full, -np.inf)
    ...
    # 4) Compute agreement and record labels
    agreement_count = 0
    neighbors = []
    for i in range(n):
        txt_nbr = int(nn_text[i])
        cov_nbrs = [int(j) for j in nn_cov_topk[i]]
        if txt_nbr in cov_nbrs:
            agreement_count += 1
        neighbors.append({
            'class': labels[i],
            'text_neighbor': labels[txt_nbr],
            'covariance_neighbors': [labels[j] for j in cov_nbrs]
        })
    
    agreement_rate = (agreement_count / n) * 100
    return agreement_rate, neighbors
...
def train(model,dino_head,projection_head,cls_means,cls_cov,labels,train_loader, test_loader, unlabelled_train_loader, textEmbed,args):
    ...
    print('Testing on unlabelled examples in the training data...')
    all_acc, old_acc, new_acc = test_on_the_fly_CA(model,dino_head,projection_head,cls_means,cls_cov,labels, unlabelled_train_loader,avg_threshold=avg_threshold_txt,std_threshold=std_threshold_txt,k=args.k,save_name='Train ACC Unlabelled',
                                                args=args)
...
def test_on_the_fly_active6(model,dino_head,projection_head, cls_means,cls_cov,labels,test_loader
    ...
    for batch_idx, (images, label, _,_) in enumerate(tqdm(test_loader)):
        ...
        mask = np.append(mask, np.array([True if x.item() in range(args.num_labeled_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_labeled_classes', type=int, default=0)
    parser.add_argument('--num_unlabeled_classes', type=int, default=0)
    ...
    for run in range(0, 1):
        ...
        print("CA,k=3.5,updated after 10 novel classes,realign epochs=5")
        args.data_root= inaturalist_root
        
        args.num_labeled_classes = 28
        args.num_unlabeled_classes = 56
        ...
        # DATASETS
        # --------------------
        dataset_train, dataset_val, test_dataset_unlabelled = build_dataset(args=args,train_transform=train_transform,test_transform=test_transform)
        ...
        # DATALOADERS
        # --------------------
        labelled_train_loader = DataLoader(dataset_train, num_workers=args.num_workers, batch_size=args.batch_size, 
                                  shuffle=True, drop_last=True)
        unlabelled_train_loader = DataLoader(test_dataset_unlabelled, num_workers=args.num_workers,
                                            batch_size=args.batch_size, shuffle=False)
        test_loader = DataLoader(dataset_val, num_workers=args.num_workers,
                                          batch_size=args.batch_size, shuffle=False)
        #text Embedding
        textEmbed=pickle.load(open("sbertArachnida.p","rb"))
        #textEmbed=pickle.load(open("sbertOxfordPets.p","rb"))
        #textEmbed=pickle.load(open("sbertFood101.p","rb"))
        textEmbed=textEmbed.T
        texts=textEmbed
        #print("text Embed: ",textEmbed.size())
        #textEmbed = textEmbed[0:80,:]
        #textEmbed = textEmbed[0:51,:]
        #textEmbed = textEmbed[0:19,:]
        ...
        cls_means2,cls_cov2,labels2=compute_mean_var_test(model,dino_head,unlabelled_train_loader)
        #print(cls_means2)
        #print(labels2)
        #print(cls_cov2)
        texts=texts.numpy()
        covs=cls_cov2.numpy()
        cls_means2=cls_means2.numpy()
        #agreement=validate_semantic_covariance_topk_alignment(texts, covs,k=7)
        #print("centroid",centroid.size())
        #centroid=centroid.cpu().numpy()
        

        #agreement2=validate_semantic_covariance_topk_alignment(cls_means2, covs,k=7)
        #print("rho",rho)
        #print("agreement",agreement)
        #print("hi")
        #print("rho2",rho2)
        
        #print("agreement2",agreement2)

        #avg_score = average_covariance_score_for_topk_text_neighbors(texts, covs, k= 7)
        #print(f"Average covariance similarity score for text-based neighbors: {avg_score:.3f}")
        #avg_score2 = average_covariance_score_for_topk_text_neighbors(cls_means2, covs, k= 7)
        #print(f"Average covariance similarity score for text-based neighbors: {avg_score2:.3f}")

        #scores = max_covariance_score_for_topk_text_neighbors(texts, covs, k=7)
        #avg_score=np.mean(scores)
        #for idx, score in enumerate(scores):
        #    print(f"Class {idx}: max covariance similarity with top-3 text neighbors = {score:.3f}")
        #print("avg_score",avg_score)
        #labels=list(range(n))
        #agreement_rate, neighbors = validate_alignment_top3_with_labels(cls_means2, covs, labels=None, k=3)
        #print(f"Agreement rate (top-1 text in top-3 covariance): {agreement_rate:.1f}%")
