        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                for i in range(f1.size(0)):
                    ...
                    #Mean
                    temp_cos=mcos_sim[i,:]
                    values,idx= torch.topk(temp_cos, dim=-1, largest=True, k=2)                   
                    temp=mcos_sim[i,index]
                    margin=values[0]-values[1]
                    
                    #print("value",temp)
                    margin_sim[index.item()].append(margin.item())
                    cosine_sim_image[index.item()].append(temp.item())
                    mean_cos_val[index]=mean_cos_val[index]+temp
            mean_cos_val=mean_cos_val.to(device)
            mean_cos_val=mean_cos_val/count_samples
            #margin_sim=margin_sim/count_samples
            #print("margin_sim:",margin_sim)
            margin_min=torch.zeros(textEmbed.size(0))
            margin_std=torch.zeros(textEmbed.size(0))
            margin_mean=torch.zeros(textEmbed.size(0))
            avg_threshold_txt=torch.zeros(textEmbed.size(0))  
            std_threshold_txt=torch.zeros(textEmbed.size(0))   
            ...
            for i in range(textEmbed.size(0)):
                ...
                temp_margin=torch.tensor(margin_sim[i])
                me=torch.mean(temp_margin)
                ms=torch.std(temp_margin)
                mi=torch.min(temp_margin)
                margin_mean[i]=me
                margin_std[i]=ms
                margin_min[i]=mi
            ...
            print("std margin",margin_std)
            print("min margin",margin_min)
            print("mean margin",margin_mean)    
            print("mean size",centroid.size())
            margin_threshold=margin_mean-4*margin_std
                ...
                if con_val>=0:
                    ...
                    if(bud<budget):
                        ...
                        if(count_novel-count)==3:
                            ...
                            textEmbed=neo_protos
        if(c_sample%1000==0):
            print("count_samples",c_sample)
            print("count_novel",count_novel) 
...
def test_on_the_fly_entropy_baseline(model,dino_head,projection_head, cls_means,cls_cov,labels,test_loader
                        ,avg_threshold,std_threshold,margin_mean,margin_std,k,save_name,args):
...
#    print("len of cls_means before testing",len(cls_means))
    ...
    preds = []
    textEmbedNovel={}
    for i in range(50*textEmbed.size(0)):
        textEmbedNovel[i]=[]
    ...
    for feat in feat_list:
        ...
        s=torch.sum(cos_sim)
        #print("cos_sim sum",s)
        entropy = torch.sum(-1*cos_sim * torch.log2(cos_sim))
...
if __name__ == "__main__":
    ...
    for run in range(0, 1):
        ...
        print(" CA mean-k*std,k=2.5,updated after 6 novel classes,realign epochs=10")
        ...
        #agreement=validate_semantic_covariance_topk_alignment(texts, covs,k=5)
        #print("centroid",centroid.size())
        #centroid=centroid.cpu().numpy()

        #agreement2=validate_semantic_covariance_topk_alignment(cls_means2, covs,k=5)
        #print("rho",rho)
